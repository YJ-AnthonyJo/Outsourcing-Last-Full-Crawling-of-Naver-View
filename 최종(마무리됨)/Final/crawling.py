...
#    display.start()
    ...
    writtenby = [k.get_text() for k in soup.find_all(attrs={'class': 'sub_txt sub_name'})]
    #print(writtenby)

    print('Data개수 :', len(Urls), len(rank), len(title), len(date), len(writtenby))
    driver.close()
#for ubuntu
    #display.stop()

    return [Urls, writtenby, title, date, rank]
